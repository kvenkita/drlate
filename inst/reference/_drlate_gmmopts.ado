...
capture program drop _drlate_gmmopts
program define _drlate_gmmopts, sclass
	version 17
	...
	if "`iterate'"!="" {
		...
		local iteropt conv_maxiter(`iterate')
	...
	if "`nrtolerance'"!="" {
		if "`technique'"=="" {
			di as err "{p}option {bf:nrtolerance()} is not allowed; " ///
				"{bf:drlate} uses the Gauss-Newton algorithm{p_end}"
			exit 198
		}
	...
	if "`conv_nrtol'"!="" {
		if "`technique'"=="" {
			di as err "{p}{bf:gmm} option {bf:conv_nrtol()} is not " ///
				"allowed; {bf:drlate} uses {bf:gmm} and the " ///
...
program define ParseVCE, sclass
	...
	if `"`r(cluster)'"'!="" {
		...
		sreturn local vcetype Robust
		sreturn local vceopts `vce' `r(cluster)'
	}
