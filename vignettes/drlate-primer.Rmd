...
## Abadie's kappa and the weighting-estimator menu

The companion paper Słoczyński, Uysal, and Wooldridge (2025) studies the
family of LATE estimators motivated by Abadie's (2003) kappa theorem.
`drlate` implements the full menu of their Stata command `kappalate`:
...
The paper recommends the normalized estimators — especially `tau_u`, which
is `method = "ipw"` here — because the unnormalized variants are sensitive
to the location and scale of the outcome. The kappa methods use no outcome
or treatment model, so the formulas must be intercept-only:
...
Fieller confidence sets (`"kappa"` and `"kappa0"`; `"kappa10"` is a
difference of two ratios, so use the bootstrap there).
