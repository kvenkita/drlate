...
the original Stata command of the same name — implements. Along the way it
...
effect for compliers is **0.5**. Two things are deliberately wrong with
the naive approaches: the treatment is genuinely endogenous (people who
...
Estimates deserve to be accompanied by evidence that the design behind
them is sound. `plot()` provides the three standard displays.
...
Note how `age` moves from a 0.45 standardized difference to essentially
zero after weighting.
...
interval, it keeps its promised coverage no matter how weak the
instrument — at the honest price that it may be **unbounded** when the
data barely identify the ratio.
...
answer) while the Wald interval degenerated. See the validation report on
the package website.
...
standard errors from zero. In practice: a rejection says you really did
need the instrument; a non-rejection says the controls may suffice — and
the much more precise unconfoundedness-based estimator becomes
defensible.
