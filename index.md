...
Wooldridge in one package: the doubly robust IPWRA/IPW/AIPW/RA
estimators of [Słoczyński, Uysal & Wooldridge
(2022)](https://doi.org/10.48550/arXiv.2208.01300) and the Abadie-kappa
weighting estimators of [Słoczyński, Uysal & Wooldridge (2025,
*JBES*)](https://doi.org/10.1080/07350015.2024.2332763), behind a single
three-formula interface. Standard errors for every estimator come from
one jointly stacked M-estimation system whose sandwich variance accounts
for all estimation stages, and the test suite verifies numerical
equivalence of estimates *and* standard errors against fixtures
generated by the authors’ own Stata commands
([`drlate`](https://ideas.repec.org/c/boc/bocode/s459708.html), SSC
S459708, and
[`kappalate`](https://ideas.repec.org/c/boc/bocode/s459257.html), SSC
S459257) across 38 scenarios. Around the estimators the package provides
the workflow applied IV analysis needs: design diagnostics,
...
- The Abadie-kappa weighting menu from Słoczyński, Uysal & Wooldridge
  ...
  `kappalate`’s `tau_u` and `tau_a,1`, completing the five-estimator
  menu.
...
- Probit instrument propensity scores (`ivmodel = "probit"`, mirroring
  `kappalate`’s `zmodel(probit)`) for the weighting estimators. With
  this, every estimator and propensity-score option of the Stata
  `kappalate` command is covered and fixture-validated.
- Printed output shows the `kappalate` estimator names throughout.

See [NEWS](https://kvenkita.github.io/drlate/NEWS.md) for the full
changelog.
...
> Słoczyński, T., Uysal, S. D., & Wooldridge, J. M. (2022). Doubly
...
| Diagnostics | `plot(fit)` for propensity-score overlap (histogram or density), covariate balance (love plot or per-covariate densities), and weight distributions; [`balance()`](https://kvenkita.github.io/drlate/reference/balance.md) / `balance(fit, detail = TRUE)` tables; [`balance_test()`](https://kvenkita.github.io/drlate/reference/balance_test.md) (Imai–Ratkovic overidentification balance test); first-stage strength on every printout |
| Complier profiling | [`complier_means()`](https://kvenkita.github.io/drlate/reference/complier_means.md) for population-vs-complier covariate means; [`kappa_weights()`](https://kvenkita.github.io/drlate/reference/kappa_weights.md) for the underlying Abadie-kappa weights |
...
| DR Hausman test | [`dr_hausman()`](https://kvenkita.github.io/drlate/reference/dr_hausman.md) — test of unconfoundedness under one-sided noncompliance (2022 paper, Section 5) |
